    ...
    public class ManageJobOpenings
    {
        public JobDescription ManageJobs(JobDescription jobDtl)
        {
            try
            {
                ...
                IEmployers employer = new Jobs(HttpContext.Current.Request.Cookies["talentsprofile_user"]);
                ...
                employer.ManageJobs(jobDtl.JobId, string.Empty, string.Empty, jobDtl.JobRefCode, jobDtl.JobTitle, jobDtl.DesignationFor,
                    ...
                    jobDtl.ExpireOn, jobDtl.Education, jobDtl.SalaryType, jobDtl.PublishOn, jobDtl.JobType, jobDtl.IsWalkIn, jobDtl.OrganizationId);
            }
            catch (Exception ex)
            {
                // Common.LogError(ex);
